...
module.exports = __webpack_require__("cs/S");
...
/* harmony import */ var _services_supabase__WEBPACK_IMPORTED_MODULE_2__ = __webpack_require__("TZoU");
  ...
  callbacks: {
    ...
    async signIn(user, account, profile) {
      ...
      const {
        data: users,
        error
      } = await _services_supabase__WEBPACK_IMPORTED_MODULE_2__[/* supabase */ "a"].from('users').select('*').eq('email', email).single(); // if theres a user with the same email as the current user, users.lengh will be 1
      ...
      if ((users === null || users === void 0 ? void 0 : users.length) === 0 || users === null) {
        ...
        await _services_supabase__WEBPACK_IMPORTED_MODULE_2__[/* supabase */ "a"].from('users').insert([{
          name: `${name}`,
          avatar_url: `${image}`,
          email: `${email}`
        }]);
